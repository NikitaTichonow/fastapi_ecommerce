...
async def product_dy_category(db: Annotated[AsyncSession, Depends(get_db)], category_slug: str):
    ...
    categories_and_subcategories = [
        category.id] + [i.id for i in subcategories]
    products_category = await db.scalars(select(Product).where(Product.category_id.in_(
        categories_and_subcategories), Product.is_active == True, Product.stock > 0))
